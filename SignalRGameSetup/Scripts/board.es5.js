...
function updateEnterGameButton() {
    ...
    if (isStarter) {
        ...
        if (gameSetup.Players.length < minimumPlayers) {
            ...
            var howManyLeft = minimumPlayers - gameSetup.Players.length;
            if (howManyLeft === 1) {
...
function updateWaitRoomPlayers() {
    ...
    for (var i = 0; i < gameSetup.Players.length; i++) {
...
function updateWaitRoomWatchers() {
    ...
    // loop through list of players in the setup
    for (var i = 0; i < gameSetup.Watchers.length; i++) {
...
yesAudienceBtn.addEventListener("click", function () {
    newRoom(true);
});
noAudienceBtn.addEventListener("click", function () {
    newRoom(false);
});

joinRoomBtn.addEventListener("click", askForGameCode);
submitGameCodeBtn.addEventListener("click", function () {
    decideHowToEnter();
});
...
audienceModalClose.addEventListener("click", function () {
    allowAudienceModal.style.display = "none";
});
